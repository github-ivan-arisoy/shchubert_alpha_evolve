...
================================
IMPORTANT: 
 - ❗❗❗ DON'T WORRY Ivan indeed Can provide you a .py file with the code ❗❗❗ 
 - 🤨 NO .ipynb FILES WERE INVOLVED NOR INCLUDED IN HERE! 🤨

DISCLAIMER: 
- 😵 THIS FILE WILL CONTAIN A LOT OF WELL SELECTED EMOJIS AND COMMENTS GENERATED BY OPUS 4.5 TURBO 120B 🤖
- ⚠️⚠️⚠️ FOR SENSITIVE LeCHAT USERS : IF IT IS TOO OVERWHELMING, PLEASE CONTACT IVAN. HE CAN REMOVE THE EMOJIS AND COMMENTS. ⚠️⚠️⚠️
"""
...
# =============================================================================
# ⚙️  CONFIGURATION - EDIT YOUR PARAMETERS HERE  ⚙️
# =============================================================================
#
...
# -----------------------------------------------------------------------------
# 📚  STEP 1: PRE-TRAINING (MLM + NT-Xent Contrastive Learning)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 🎯  STEP 2: FINE-TUNING (CTR Binary Classification)
# -----------------------------------------------------------------------------
# Folder structure:
...
# -----------------------------------------------------------------------------
# 🧊  STEP 3: DOWNSTREAM (Frozen Backbone + New Head)
# -----------------------------------------------------------------------------
# Folder structure:
...
class CSVLoggingCallback(keras.callbacks.Callback):
    ...
    def on_train_begin(self, logs=None):
        ...
        logger.info(f"📝 Metrics will be logged to: {self.csv_path}")
...
def shift_bias_for_negative_sampling(
    ...
    try:
        ...
        logger.info(
            f"🔧 Bias shifted for {negative_sampling_rate}% negative sampling: "
...
class HuBertPretrainer:
    ...
    def run(self) -> str:
        ...
        logger.info("=" * 80)
        logger.info("🚀 HuBERT Pre-Training: MLM + NT-Xent Contrastive Learning")
        logger.info("=" * 80)

        logger.info("📊 Step 1/3: Loading pre-training dataset...")
        dataset = self.generate_dataset()

        logger.info("🧠 Step 2/3: Building HuBERT pre-training model...")
        ...
        logger.info("🏋️ Step 3/3: Training HuBERT model...")
...
class HuBertFinetuner:
    ...
    def load_train_val_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if val_data is None or len(val_data) == 0:
            ...
            logger.warning(
                f"⚠️ No validation data found in val/ folder. "
    ...
    def run(self) -> str:
        ...
        logger.info("=" * 80)
        logger.info("🚀 HuBERT Fine-Tuning: CTR Binary Classification")
        logger.info("=" * 80)

        logger.info("📊 Step 1/4: Loading training and validation datasets...")
        train_dataset, val_dataset = self.generate_datasets()

        logger.info("🧠 Step 2/4: Loading pretrained backbone and building CTR head...")
        strategy = tf.distribute.MirroredStrategy(
            cross_device_ops=tf.distribute.HierarchicalCopyAllReduce()
        )
        ...
        logger.info("🏋️ Step 3/4: Training CTR HuBert Model...")
        model = self.train_model(model, train_dataset, val_dataset)

        logger.info("💾 Step 4/4: Saving fine-tuned model...")
        saved_path = self.save_model_locally(model)
...
class HuBertDownstream:
    ...
    def evaluate_on_test(
        ...
        logger.info("=" * 60)
        logger.info("📊 Test Evaluation with Bias Shift")
        logger.info("=" * 60)
    ...
    def run(self) -> str:
        ...
        logger.info("=" * 80)
        logger.info("🚀 HuBERT Downstream: Frozen Backbone + New CTR Head")
        logger.info("=" * 80)

        logger.info("📊 Step 1/6: Loading training, validation, and test datasets...")
        train_dataset, val_dataset, test_arrays = self.generate_datasets()

        logger.info("🧠 Step 2/6: Loading fine-tuned model and creating downstream head...")
        strategy = tf.distribute.MirroredStrategy(
            cross_device_ops=tf.distribute.HierarchicalCopyAllReduce()
        )
        ...
        logger.info("🏋️ Step 3/6: Training Downstream CTR Model (only new layers)...")
        model = self.train_model(model, train_dataset, val_dataset)

        logger.info("💾 Step 4/6: Saving downstream model...")
        saved_path = self.save_model_locally(model)
        ...
        if test_arrays is not None:
            ...
            logger.info("⏭️ Step 5/6: Skipping test evaluation (no test data)")
            test_metrics = None

        logger.info("=" * 80)
        logger.info("✅ Downstream Training Complete!")
        logger.info(f"📦 Model saved to: {saved_path}")
        if test_metrics:
            logger.info(f"📊 Test AUC: {test_metrics['test_auc']:.4f}, RIG: {test_metrics['test_rig']:.4f}")
        logger.info("=" * 80)
...
Examples:
    ...
    if args.mode == "generate-dummy":
        logger.info("=" * 80)
        logger.info("🎲 Generating Dummy Data with New Folder Structure")
        logger.info("=" * 80)
    ...
    logger.info("=" * 80)
    logger.info("🎉 HuBERT Experiment Pipeline Complete!")
    logger.info("=" * 80)
